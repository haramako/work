...
;;; __reg(4,5) = __reg(0,1) * __reg(2,3)
__mul_16:
        rts						; 未実装
        
;;; int8/int8=>int8 の割り算
;;;  __reg4 = __reg0 / __reg2 ( 余り=__reg5)
__div_8:
		txa                     ; push x
		pha
        ldx #8
...
__div_8s:
		...
		lda __reg+0				; if sign(__reg0) then goto .reg0_pos
		bpl .reg0_pos
		lda #0					; reg0 = -reg0
		sec
		sbc __reg+0
...
.reg0_pos:
		...
		lda __reg+2				; if sign(__reg2) then goto .reg2_pos
		bpl .reg2_pos
		lda #0					; reg2 = -reg2
		sec
		sbc __reg+2
...
.reg2_pos:
		...
		lda __reg+6				; if reg6 then goto .else2
		beq .else1
		lda #0					; reg4 = -reg4
		sec
		sbc __reg+4				
		sta __reg+4
		lda __reg+5				; if reg5 == 0 then goto .else1
		beq .else1
		lda __reg+2				; reg5 = reg2 - reg5
		sec
		sbc __reg+5
		sta __reg+5
		dec __reg+4				; reg4 = reg4-1
.else1:
		lda __reg+7				; if reg7 then goto .else2
		beq .else2
		lda #0					; reg5 = -reg5
		sec
		sbc __reg+5
...
;;;  __reg(4,5) = __reg(0,1) / __reg(2,3) ( 余り=__reg(6,7))
__div_16:
		txa                     ; push x
		pha
        ldx #8
