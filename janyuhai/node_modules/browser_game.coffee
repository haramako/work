...
class Client
    constructor: (pid)->
        ...
        @messageId = 0
        @pid = pid
        @ws = new WebSocket("ws://localhost:20001/#{@pid}", 'janyuhai-game')
...
startIndexHtml = ->
    $(document).ready ->
        window.admin = new AdminClient 'localhost',20001, (conn)->
            reloadSession()
...
reloadSession = ->
    window.admin.send {t:'SESSION_LIST'}, (msg)->
        ...
        for s in msg.sessions
            ...
            html += '<a href="game.html?host=localhost:20001&pid='+pid+'">プレイヤー'+i+'</a>　' for pid,i in s.players
            html += '</td></tr>'
            table.append( $(html) )
